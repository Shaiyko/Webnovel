...
export default function SearchAuthor() {
  ...
  const displayData = author.slice(
    (page - 1) * itemsPerPage,
    page * itemsPerPage
  );
  ...
  const fetchNovels = async () => {
    ...
    try {
      ...
      setAuthorname(response.data[0].penname);
    } catch (error) {
  ...
  return (
    <>
      <Card sx={{ minWidth: 275, mb: 1 }}>
        <CardContent>
          <Box>
            <Typography>Search Author: {`"${penname}"`}</Typography>
          </Box>
        </CardContent>
      </Card>
      <Card sx={{ minWidth: 275 }}>
        <CardContent>
          ...
          >
            <Pagination
              count={Math.ceil(author.length / itemsPerPage)}
              page={page}
