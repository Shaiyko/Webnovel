...
import ViewAdmin from "./ViewUser";
...
export default function TableUse() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = dataTag.map((n) => n.id_user);
  ...
  //
  const handleClick = (event, id_user) => {
    const selectedIndex = selected.indexOf(id_user);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, id_user);
  ...
  const isSelected = (id_user) => selected.indexOf(id_user) !== -1;
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row) => {
                ...
                const labelId = `enhanced-table-checkbox-${row.id_user}`;
