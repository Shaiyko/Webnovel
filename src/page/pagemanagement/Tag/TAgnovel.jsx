...
export default function EnhancedTable() {
  ...
  const [orderBy, setOrderBy] = useState("id_taek");
  ...
  const filteredRows = useMemo(() => {
    return dataTag.filter((row) =>
      row.name_taek.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [dataTag, searchQuery]);
      ...
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row) => {
                const isItemSelected = isSelected(row.id_taek);
                const labelId = `enhanced-table-checkbox-${row.id_taek}`;

                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, row.id_taek)}
                    role="checkbox"
                  ...
                  >
                    ...
                    <TableCell align="right">{row.name_taek}</TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
